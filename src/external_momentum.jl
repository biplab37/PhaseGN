...
function imagpart_phi_q_refactored_m(ω, temp, μ, q, m, param)
    if q==0.0
        return imagpart_phi(ω, temp, μ, m, param)
    end
...
function imagpart_sigma_q_refactored_m(ω, temp, μ, q, m, param)
    if q == 0.0
        return imagpart_sigma(ω, temp, μ, m, param)
    end
    ...
    if s < 0.0
        return imagpart_sigma_q_s_negative(ω, temp, μ, q, m, s, ω_max)
    else # s > 4m^2
        return imagpart_sigma_q_s_positive(ω, temp, μ, q, m, s, ω_max)
    end
end
...
function imagpart_sigma_q_s_negative(ω, temp, μ, q, m, s, ω_max)
    y = q*sqrt(1 - 4*m^2/s)
    if y >= ω_max
        return 0.0
    end
    function integrand_1(E1)
        discriminant = (s-4*m^2)*q^2 - s*E1^2
        if discriminant > 0.0
            return (s - 4*m^2)*(pauli_blocking_q_1(temp, μ, E1, ω) + pauli_blocking_q_1(temp, μ, -E1, ω)) / (sqrt(discriminant)*4π)
        else
            return 0.0
        end
    end
...
function imagpart_phi_q_s_positive(ω, temp, μ, q, m, s, ω_max)
    ...
    function integrand_2(E2)
        discriminant = (s-4*m^2)*q^2 - s*E2^2
        if discriminant > 0.0
...
function imagpart_sigma_q_s_positive(ω, temp, μ, q, m, s, ω_max)
    y = q*sqrt(1 - 4*m^2/s)
    if y < 1e-5
        return 0.0
    end
    ...
    function integrand_2(E2)
        discriminant = (s-4*m^2)*q^2 - s*E2^2
        if discriminant > 0.0
            return (s - 4*m^2)*pauli_blocking_q_1(temp, μ, E2, ω) / (sqrt(discriminant)*4π)
        else
            return 0.0
        end
    end
