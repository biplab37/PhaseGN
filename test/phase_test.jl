...
@testset "Scalar Channel" begin
    ...
    @test phasesc_sigma(temp,mu,rand(),m,param) ≈ phasesc_sigma(temp,mu,rand(),param) atol=1e-3
    @test phaser_sigma(temp,mu,rand(),m,Π00sigma, param) ≈ phaser_sigma(temp,mu,rand(),param) atol=1e-3
end
...
@testset "Pseudo-Scalar Channel" begin
    ...
    @test phasesc_phi(temp,mu,rand(),m,param) ≈ phasesc_phi(temp,mu,rand(),param) atol=1e-3
    @test phaser_phi(temp,mu,rand(),m,Π00phi,param) ≈ phaser_phi(temp,mu,rand(),param) atol=1e-3
end